SamacSys ECAD Model
13001891/1743273/2.50/2/2/TVS Diode Uni-directional

DESIGNSPARK_INTERMEDIATE_ASCII

(asciiHeader
	(fileUnits MM)
)
(library Library_1
	(padStyleDef "r220_215"
		(holeDiam 0)
		(padShape (layerNumRef 1) (padShapeType Rect)  (shapeWidth 2.150) (shapeHeight 2.200))
		(padShape (layerNumRef 16) (padShapeType Ellipse)  (shapeWidth 0) (shapeHeight 0))
	)
	(textStyleDef "Default"
		(font
			(fontType Stroke)
			(fontFace "Helvetica")
			(fontHeight 50 mils)
			(strokeWidth 5 mils)
		)
	)
	(patternDef "SMBJ12A" (originalName "SMBJ12A")
		(multiLayer
			(pad (padNum 1) (padStyleRef r220_215) (pt -2.200, 0.000) (rotation 90))
			(pad (padNum 2) (padStyleRef r220_215) (pt 2.200, 0.000) (rotation 90))
		)
		(layerContents (layerNumRef 18)
			(attr "RefDes" "RefDes" (pt 0.000, 0.000) (textStyleRef "Default") (isVisible True))
		)
		(layerContents (layerNumRef 30)
			(line (pt -3.55 2.225) (pt 3.55 2.225) (width 0.05))
		)
		(layerContents (layerNumRef 30)
			(line (pt 3.55 2.225) (pt 3.55 -2.225) (width 0.05))
		)
		(layerContents (layerNumRef 30)
			(line (pt 3.55 -2.225) (pt -3.55 -2.225) (width 0.05))
		)
		(layerContents (layerNumRef 30)
			(line (pt -3.55 -2.225) (pt -3.55 2.225) (width 0.05))
		)
		(layerContents (layerNumRef 28)
			(line (pt -2.675 1.812) (pt 2.675 1.812) (width 0.1))
		)
		(layerContents (layerNumRef 28)
			(line (pt 2.675 1.812) (pt 2.675 -1.812) (width 0.1))
		)
		(layerContents (layerNumRef 28)
			(line (pt 2.675 -1.812) (pt -2.675 -1.812) (width 0.1))
		)
		(layerContents (layerNumRef 28)
			(line (pt -2.675 -1.812) (pt -2.675 1.812) (width 0.1))
		)
		(layerContents (layerNumRef 18)
			(line (pt -2.675 -1.812) (pt 2.675 -1.812) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(line (pt -2.675 1.812) (pt 2.675 1.812) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(line (pt -0.75 1.812) (pt -0.75 -1.812) (width 0.2))
		)
	)
	(symbolDef "SMBJ5_0A-TR" (originalName "SMBJ5_0A-TR")

		(pin (pinNum 1) (pt 0 mils 0 mils) (rotation 0) (pinLength 200 mils) (pinDisplay (dispPinName false)) (pinName (text (pt 0 mils -45 mils) (rotation 0]) (justify "UpperLeft") (textStyleRef "Default"))
		))
		(pin (pinNum 2) (pt 600 mils 0 mils) (rotation 180) (pinLength 200 mils) (pinDisplay (dispPinName false)) (pinName (text (pt 600 mils -45 mils) (rotation 0]) (justify "UpperRight") (textStyleRef "Default"))
		))
		(line (pt 200 mils 0 mils) (pt 400 mils 100 mils) (width 6 mils))
		(line (pt 400 mils 100 mils) (pt 400 mils -100 mils) (width 6 mils))
		(line (pt 400 mils -100 mils) (pt 200 mils 0 mils) (width 6 mils))
		(line (pt 200 mils -80 mils) (pt 240 mils -100 mils) (width 6 mils))
		(line (pt 200 mils -80 mils) (pt 200 mils 80 mils) (width 6 mils))
		(line (pt 160 mils 100 mils) (pt 200 mils 80 mils) (width 6 mils))
		(attr "RefDes" "RefDes" (pt 400 mils 350 mils) (justify LowerLeft) (isVisible True) (textStyleRef "Default"))

	)
	(compDef "SMBJ5.0A-TR" (originalName "SMBJ5.0A-TR") (compHeader (numPins 2) (numParts 1) (refDesPrefix D)
		)
		(compPin "1" (pinName "K") (partNum 1) (symPinNum 1) (gateEq 0) (pinEq 0) (pinType Bidirectional))
		(compPin "2" (pinName "A") (partNum 1) (symPinNum 2) (gateEq 0) (pinEq 0) (pinType Bidirectional))
		(attachedSymbol (partNum 1) (altType Normal) (symbolName "SMBJ5_0A-TR"))
		(attachedPattern (patternNum 1) (patternName "SMBJ12A")
			(numPads 2)
			(padPinMap
				(padNum 1) (compPinRef "1")
				(padNum 2) (compPinRef "2")
			)
		)
		(attr "Mouser Part Number" "511-SMBJ5.0A")
		(attr "Mouser Price/Stock" "https://www.mouser.co.uk/ProductDetail/STMicroelectronics/SMBJ5.0A-TR?qs=QR0jAJXDF2ih8aaSsBQmUg%3D%3D")
		(attr "Manufacturer_Name" "STMicroelectronics")
		(attr "Manufacturer_Part_Number" "SMBJ5.0A-TR")
		(attr "Description" "ESD Suppressors / TVS Diodes 600W 5.0V Unidirect")
		(attr "Datasheet Link" "https://www.st.com/resource/en/datasheet/smbj58ca.pdf")
		(attr "Height" "2.45 mm")
	)

)
